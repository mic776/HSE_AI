...
_document.onkeydown = function(ev){
	if (window.__QUIZ_PAUSED) { return; }
	_temp = ev.keyCode;
...
window.addEventListener('message', function(ev) {
	...
	if (ev.data.type === 'quiz_pause') {
		...
		window.__QUIZ_PAUSED = nextPaused;
		return;
	}
	...
	if (ev.data.type === 'quiz_key') {
		...
		var keyCodeByLayout =
			ev.data.code === 'KeyW' ? 87 :
			ev.data.code === 'KeyA' ? 65 :
			ev.data.code === 'KeyS' ? 83 :
			ev.data.code === 'KeyD' ? 68 :
			ev.data.code === 'ArrowUp' ? 38 :
		...
		if (!keyCode && keyCodeByLayout) { keyCode = keyCodeByLayout; }
		if (!keyCode) { return; }
		keyCode = key_convert[keyCode] || keyCode;
		if (keys[keyCode] !== udef) {
			keys[keyCode] = kind === 'keydown' ? 1 : 0;
		}
	}
});
